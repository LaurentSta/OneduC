...
@section('admin')
{{-- Intégration dans admin_dashboard --}}
    ...
    <div class="bg-white rounded-[20px] shadow-md p-8 w-full border border-gray-100">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 border-b pb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Module : <span class="font-semibold text-gray-800">{{ $lecture->lecture_title }}</span></p>
            </div>
            <a href="{{ route('admin.quiz.questions.index', ['lecture' => $lecture->id]) }}" class="text-bleuone hover:underline text-sm font-medium">
                &larr; Retour à la banque de questions
            </a>
        </div>

        <form method="POST" action="{{ route('admin.quiz.questions.store', ['lecture' => $lecture->id]) }}" enctype="multipart/form-data" class="space-y-8">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <template x-for="(label, key) in types" :key="key">
                            <button type="button" @click="changeType(key)"
                                ...
                                class="flex items-center justify-center p-4 border-2 rounded-xl transition-all duration-200 group">
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-3">
                    <template x-for="(option, index) in options" :key="index">
                        <div class="flex items-center gap-4 p-4 rounded-xl border border-gray-200 bg-white shadow-sm transition-all">
                            {{-- Checkbox / Radio --}}
                            <div class="flex items-center justify-center">
                                <input :type="currentType === 'multiple' ? 'checkbox' : 'radio'" 
                                       :name="currentType === 'multiple' ? `options[${index}][is_correct]` : 'correct_choice_index'"
                                       :value="currentType === 'multiple' ? '1' : index"
            ...
            {{-- Section 3 : Médias & Accessibilité (optionnel / repliable) --}}
...
function quizManager() {
    return {
        ...
        changeType(type) {
            ...
            } else if (this.options.length < 2 || (this.options[0].text === 'Vrai')) {
                this.options = [
