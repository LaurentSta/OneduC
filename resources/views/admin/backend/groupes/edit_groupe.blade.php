...
@section('admin')
    @php
        $stagiairesSelectionnes = collect(old('stagiaires', $groupe->students->pluck('id')->all()))
            ->map(fn ($id) => (int) $id)
            ->all();
    @endphp
    ...
    <div class="space-y-4">
        <header class="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <nav aria-label="Fil d’Ariane" class="text-xs font-medium text-slate-500">
                    ...
                    <span aria-current="page">{{ $groupe->name }}</span>
                </nav>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-bleuone">Modifier le groupe</h1>
                <p class="mt-1 text-sm text-slate-600">Mettez à jour le responsable et les affectations de <span class="font-semibold text-slate-800">{{ $groupe->name }}</span>.</p>
            </div>

            <div class="flex items-center gap-2 text-xs text-slate-500">
                <i class="ti ti-clock" aria-hidden="true"></i>
                Dernière mise à jour : {{ $groupe->updated_at?->format('d/m/Y à H:i') ?? '—' }}
            </div>
        </header>
        ...
        <form action="{{ route('admin.groupes.update', $groupe->id) }}" method="POST" class="grid gap-4 lg:grid-cols-12">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach ($formateurs as $formateur)
                                ...
                                @endphp
                                <option value="{{ $formateur->id }}" @selected((int) old('formateur_id', $groupe->instructor_id) === (int) $formateur->id)>
            ...
            >
                <div class="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-500">Cochez les comptes à conserver ou à ajouter dans ce groupe.</p>
                    </div>
            ...
            <footer class="flex flex-col-reverse gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-end lg:col-span-12">
                ...
                <button type="submit" class="btn-oneduc inline-flex items-center justify-center gap-2 !px-4 !py-2.5 !text-sm">
