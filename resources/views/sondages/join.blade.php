...
<head>
  ...
  <title>Oneduc - Sondage</title>
  @vite(['resources/css/app.css'])
</head>
<body class="min-h-screen bg-slate-100 flex items-center justify-center p-4">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-6">
    <h1 class="text-xl font-semibold text-[#004461]">Rejoindre un sondage</h1>
    <p class="text-sm text-gray-600 mt-1">Entrez le code donné par votre formateur.</p>

    <form method="POST" action="{{ route('sondages.resolve') }}" class="mt-6 space-y-4">
      @csrf
