...
@php
  ...
  $parcoursModulesPayload = ($mesParcours ?? collect())->mapWithKeys(function ($parcours) use ($availableModulesById, $group) {
    $modules = $parcours->items
      ->filter(fn($item) => $item->type === 'module' && $item->module_id)
      ->map(function ($item) use ($availableModulesById, $group) {
        $meta = $availableModulesById->get($item->module_id) ?? [];
        ...
        return [
          ...
          'persisted'      => $isPersisted,
          'manage_url'     => $isPersisted
            ? route('formateur.groupes.modules.lecons.edit', ['group' => $group->id, 'module' => $item->module_id])
            : '',
  ...
  $oldEmargementEnabled = old('emargement_enabled', $group->emargement_enabled);
        ...
        <div class="flex flex-wrap items-center gap-3 mb-4 font-varela text-sm">
            ...
            @if($group->observers->isNotEmpty())
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                    ...
                    <span class="font-bold">{{ $group->observers->count() }}</span> Observateur{{ $group->observers->count() > 1 ? 's' : '' }}
                </div>
            @endif
        </div>
...
Lucas;Bernard;lucas.bernard@entreprise.fr</pre>
      ...
      <section x-show="activeTab === 'parcours'" x-cloak
        ...
        {{-- Bannière parcours lié --}}
        @if($group->formateur_parcours_id && $group->formateurParcours)
          <div class="flex items-center gap-4 rounded-[12px] border border-[#004461]/20 bg-[#004461]/5 px-4 py-3 mb-4">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#004461]/10 text-[#004461]">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div
          ...
          data-initial-parcours-id="{{ $group->formateur_parcours_id ?? '' }}"
      ...
      @php
        $wcItems = $group->formateurParcours?->items->where('type', 'wordcloud') ?? collect();
      @endphp
      @if($wcItems->isNotEmpty())
        <section class="bg-white rounded-[20px] shadow-md overflow-hidden mt-6">
          <div class="bg-amber-500 px-6 py-4 flex items-center gap-3">
            ...
            <p class="text-sm font-bold text-white">Nuages de mots du parcours</p>
            <span class="ml-auto inline-flex items-center rounded-full bg-white/20 px-2.5 py-0.5 text-xs font-semibold text-white">
              {{ $wcItems->count() }} activité{{ $wcItems->count() > 1 ? 's' : '' }}
            </span>
          </div>
          <div class="divide-y divide-gray-100">
            @foreach($wcItems as $wcItem)
              <div class="flex items-center gap-4 px-6 py-4">
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-semibold text-gray-900 truncate">{{ $wcItem->wc_title }}</p>
                  <p class="text-xs text-gray-500 mt-0.5">
                    {{ count($wcItem->wc_questions ?? []) }} question{{ count($wcItem->wc_questions ?? []) > 1 ? 's' : '' }}
                    @if($wcItem->wc_duration)
                      · {{ $wcItem->wc_duration }} min
                    @endif
                  </p>
                </div>
                <a href="{{ route('formateur.groupes.wordcloud.live', [$group, $wcItem]) }}"
                   class="shrink-0 inline-flex items-center gap-1.5 rounded-[8px] bg-amber-500 px-3 py-1.5 text-xs font-bold text-white hover:bg-amber-600 transition">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
