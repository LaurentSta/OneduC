...
<div class="w-full px-6 lg:px-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">
          Quatre étapes : groupe, sondage, construction, lancement.
        </p>
      </div>
    </div>
  </header>
  ...
  <div class="bg-white rounded-[20px] shadow-md p-8 max-w-xl mx-auto mb-8"
       x-data="{
         step: {{ (int) session('sondage_wizard_step', 1) }},
         ...
         questionnaireId: '{{ session('sondage_wizard_questionnaire_id', '') }}',
         questionnaireLabel: '{{ session('sondage_wizard_questionnaire_title', '') }}',
         questionnaireCount: {{ (int) session('sondage_wizard_questionnaire_count', 0) }},
         selectGroup(id, name) { this.groupId = id; this.groupLabel = name; },
         selectQuestionnaire(q) { this.questionnaireId = q.id; this.questionnaireLabel = q.title; this.questionnaireCount = q.count; },
         startNewQuestionnaire() { this.questionnaireId = ''; this.questionnaireLabel = ''; this.questionnaireCount = 0; this.step = 3; },
       }">
    ...
    <div x-show="step === 1" x-cloak>
      ...
      <p class="text-xs text-gray-500 text-center mb-5">Qui doit pouvoir répondre à ce sondage ?</p>
    ...
    <div x-show="step === 2" x-cloak>
      <h2 class="text-lg font-bold text-bleuone text-center mb-1">Quel sondage ?</h2>
      ...
      <template x-if="questionnaires.length === 0">
        <p class="text-sm text-gray-400 text-center mb-4">Vous n'avez pas encore de sondage.</p>
      </template>
      ...
      <div class="space-y-2">
        <template x-for="q in questionnaires" :key="q.id">
          <button type="button" @click="selectQuestionnaire(q)"
            ...
            <span class="inline-flex items-center shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold"
                  :class="q.count > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
                  x-text="q.count + ' question' + (q.count > 1 ? 's' : '')"></span>
          </button>
        </template>
        ...
        <button type="button" @click="startNewQuestionnaire()"
          ...
          <span class="text-sm font-semibold text-bleuone">+ Nouveau sondage</span>
    ...
    <div x-show="step === 3" x-cloak>
      <template x-if="questionnaireId === ''">
        <div>
          <h2 class="text-lg font-bold text-bleuone text-center mb-1">Nommez votre sondage</h2>
          <p class="text-xs text-gray-500 text-center mb-5">Vous pourrez ajouter les questions juste après.</p>

          <form method="POST" action="{{ route('formateur.sondage-questionnaires.store') }}" class="flex gap-2">
            @csrf
            <input type="text" name="title" required maxlength="255" placeholder="Titre du sondage"
          ...
          <div class="mt-6">
            <button type="button" @click="step = 2" class="text-sm font-semibold text-bleuone hover:underline">← Sondage</button>
          </div>
        </div>
      </template>
        ...
        <div>
          ...
          @foreach($questionnaires as $questionnaire)
            <div x-show="questionnaireId === '{{ $questionnaire->id }}'" x-cloak>
              @include('formateur.outils._sondage_questionnaire_content', ['questionnaire' => $questionnaire, 'wizardContext' => true])
            </div>
          @endforeach

          <template x-if="questionnaireCount === 0">
            <p class="mt-3 text-xs text-amber-600">Ajoutez au moins une question pour pouvoir lancer.</p>
          </template>

          <div class="flex items-center justify-between mt-6">
            <button type="button" @click="step = 2" class="text-sm font-semibold text-bleuone hover:underline">← Sondage</button>
            <button type="button" @click="step = 4"
                    :disabled="questionnaireCount === 0"
                    :class="questionnaireCount === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-bleuone hover:bg-bleuone-light'"
                    class="rounded-[10px] px-5 py-2.5 text-sm font-bold text-white transition">
              Continuer →
            </button>
          </div>
    ...
    <div x-show="step === 4" x-cloak>
      ...
      <p class="text-xs text-gray-500 text-center mb-5">Vérifiez avant de démarrer le sondage.</p>
      ...
      <div class="rounded-xl border border-gray-200 divide-y divide-gray-100 mb-6">
        ...
        <div class="flex items-center justify-between px-4 py-3">
          <span class="text-xs font-semibold text-gray-500">Sondage</span>
          <span class="text-sm font-bold text-gray-800" x-text="questionnaireLabel + ' (' + questionnaireCount + ' question' + (questionnaireCount > 1 ? 's' : '') + ')'"></span>
        </div>
      </div>

      <form method="POST" action="{{ route('formateur.sondages.launch') }}">
        @csrf
        <input type="hidden" name="group_id" :value="groupId">
        <input type="hidden" name="poll_questionnaire_id" :value="questionnaireId">
      ...
      @foreach($sessions as $session)
        <div class="bg-white rounded-[20px] shadow-md p-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 mb-0.5">
              <p class="text-sm font-bold text-gray-900 truncate">
                {{ $session->questionnaire?->title }} — {{ $session->group?->name ?? 'Tous les groupes' }}
              </p>
          ...
          <a href="{{ route('formateur.sondages.show', $session) }}"
             class="inline-flex items-center gap-1.5 rounded-[8px] bg-bleuone px-3 py-1.5 text-xs font-bold text-white hover:bg-bleuone-light transition shrink-0">
            Ouvrir
          </a>
        </div>
      @endforeach
          ...
          <div x-data="{ open: false }" class="rounded-[14px] border border-gray-200">
            <div class="flex items-center gap-3 p-3">
              <button type="button" @click="open = !open" class="flex-1 flex items-center justify-between text-left">
                ...
                <span class="text-[11px] text-gray-400 mr-2">{{ count($questionnaire->questions ?? []) }} question(s)</span>
              </button>
              <form method="POST" action="{{ route('formateur.sondage-questionnaires.destroy', $questionnaire) }}"
            ...
            <div x-show="open" x-cloak class="border-t border-gray-100 p-3 space-y-2">
              @include('formateur.outils._sondage_questionnaire_content', ['questionnaire' => $questionnaire])
            </div>
          </div>
        @empty
          <p class="text-sm text-gray-400 text-center py-6">Aucun sondage créé pour l'instant.</p>
        @endforelse
      </div>
    </div>
  </details>
...
<script>
  function sondageQuestionnaireQuestionForm() {
    return {
      showForm: false,
      questionText: '',
      choices: ['', ''],
