        ...
        <form method="POST" action="{{ route('formateur.profil.store') }}" enctype="multipart/form-data">
          ...
          <div class="flex justify-center mb-6">
            <div class="relative group w-28 h-28">
              <img id="avatar" src="{{ !empty($profileData->photo) ? asset('upload/formateur_images/'.$profileData->photo) : asset('upload/NoPhoto.png') }}"
                   alt="Avatar"
...
@if (session('message'))
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (img) {
                const currentSrc = img.src.split('?')[0]; // Supprime ?v=XXX
                img.src = `${currentSrc}?v=${Date.now()}`; // Recharge l’image avec un nouveau timestamp
            }
        });
    </script>
@endif
