...
<div class="max-w-[1285px] mx-auto px-8">
  ...
  <header class="bg-white rounded-[20px] shadow-md px-8 pt-4 pb-6 w-full mb-6">
    <div class="flex items-center justify-between">
      <div>
        <x-typography variant="titre">Modifier le parcours</x-typography>
        ...
        <nav class="text-sm font-varela text-gray-600 mt-2" aria-label="Fil d'Ariane">
          <ol class="inline-flex items-center space-x-1">
            ...
            <li class="flex items-center">
              <a href="{{ route('formateur.mes-parcours.show', $parcours) }}" class="text-orangeone hover:underline">{{ Str::limit($parcours->title, 30) }}</a>
              <span class="mx-2 text-gray-400">/</span>
            </li>
      ...
      <div class="flex items-center gap-3">
        <button type="button" x-data x-on:click="$dispatch('open-modal', 'delete-parcours-{{ $parcours->id }}')"
        ...
        <x-confirm-modal
          name="delete-parcours-{{ $parcours->id }}"
          title="Supprimer définitivement ce parcours ?"
          :action="route('formateur.mes-parcours.destroy', $parcours)"
          method="DELETE"
          confirm-label="Supprimer"
        />
        <a href="{{ route('formateur.mes-parcours.show', $parcours) }}"
