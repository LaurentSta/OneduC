...
<div x-data="{ 
    ...
    <nav class="flex mb-4 text-xs font-medium" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2">
            ...
            <li class="text-gray-300">/</li>
            <li class="text-bleuone font-bold truncate max-w-[200px]">{{ $module->module_name }}</li>
        </ol>
    </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <main class="lg:col-span-8">
            ...
            {{-- Padding réduit (pb-2 mb-4) pour remonter le contenu --}}
            <header class="border-b border-gray-100 pb-2 mb-4">
                @if($module->bestseller) 
                    <span class="inline-block bg-green-100 text-green-700 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded mb-2">Bestseller</span> 
                @endif
            ...
            <div class="min-h-[100px] mb-8">
                ...
                <div x-show="activeTab === 'prerequis'" x-cloak class="bg-gray-50 p-5 rounded-xl border border-gray-100">
                    ...
                    <div class="text-gray-700 pl-6 text-sm">
                        {!! nl2br(e($module->prerequi ?? 'Aucun prérequis spécifique pour ce module.')) !!}
                    </div>
                </div>
            </div>
                        ...
                        <div class="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
                            <button @click="openSection = (openSection === {{ $idx }} ? -1 : {{ $idx }})"
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="text-xs text-gray-400 font-semibold hidden sm:inline-block">{{ $section->lectures->count() }} leçons</span>
                                    ...
                                    <a href="{{ route('stagiaire.module.lecture', [$module->id, $section->id, $lecture->id]) }}" 
                                       class="flex items-center justify-between p-3 pl-6 hover:bg-orange-50/50 transition-colors border-b border-gray-50 last:border-0 group">
                                        ...
                                        <div>
                                            ...
                                            @if($status === 'completed')
                                                <svg class="size-5 text-green-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                                            @elseif($status === 'incomplete')
                                                <svg class="size-5 text-orangeone" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" /></svg>
                                            @else
            ...
            <div class="bg-white rounded-[24px] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                ...
                <div class="w-full bg-black">
                    @if($module->module_video)
                        <video class="w-full aspect-video object-cover" controls preload="metadata" poster="{{ !empty($module->image) ? asset($module->image) : '' }}">
                ...
                <div class="p-6">
                    ...
                    <div class="mb-6">
                        <div class="flex justify-between items-end mb-2">
                            <span class="text-xs font-bold text-gray-500 uppercase">Votre avancée</span>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 space-y-5">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="font-bold text-gray-900">{{ $module->formateur->name ?? 'Équipe Oneduc' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-[10px] text-gray-400 font-bold uppercase">Durée estimée</p>
                                {{-- 
                                   PRIORITÉ D'AFFICHAGE :
                                   1. La durée calculée automatiquement (somme des leçons)
                                   2. Sinon, la durée manuelle (champ 'duree' du module)
                                   3. Sinon, le texte par défaut "Rythme libre"
                                --}}
                                <p class="font-bold text-gray-900">
                                    {{ $module->formatted_duration ?? $module->duree ?? 'Rythme libre' }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="font-bold text-gray-900">{{ $module->sections->flatMap->lectures->count() }} leçons</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-[10px] text-gray-400 font-bold uppercase">Accès</p>
                                <p class="font-bold text-gray-900">Illimité à vie</p>
                            </div>
                        </div>
    ...
    <div x-show="showEvalModal" 
        ...
        <div @click="showEvalModal = false" class="absolute inset-0 bg-bleuone/60 backdrop-blur-sm transition-opacity"></div>
        ...
        <div class="relative bg-white rounded-[32px] shadow-2xl max-w-lg w-full p-8 overflow-hidden transform transition-all">
            ...
            <div class="text-center">
                ...
                <h3 class="text-2xl font-black text-bleuone">Prêt pour la certification ?</h3>
                <p class="mt-3 text-gray-600">
                    Vous êtes sur le point de lancer l'examen final de<br><strong>{{ $module->module_name }}</strong>.
                </p>
