  ...
  <div class="bg-white rounded-[20px] shadow-md p-8">
      ...
      @if (!empty($question->image_path))
        <figure class="mt-4">
          <img
            ...
            class="max-h-[320px] rounded-xl border border-gray-200"
          />
        </figure>
      @endif
    ...
    <form method="POST" action="{{ route('stagiaire.lesson.quiz.answer', ['module'=>$module->id,'section'=>$section->id,'lecture'=>$lecture->id,'attempt'=>$attempt->id]) }}">
      ...
      <div class="flex justify-end pt-6">
        <button type="submit" class="inline-flex items-center px-8 py-3 bg-orangeone text-white font-semibold rounded-xl hover:opacity-90 transition">
          Valider
        </button>
      </div>
    </form>
