{{-- SIDEBAR --}}
<aside
  class="w-full md:w-72 xl:w-80 bg-white rounded-[20px] shadow-md p-5 md:sticky md:top-4 h-max"
      ...
      <div class="flex items-center gap-2">
        ...
        <a href="{{ route('stagiaire.module.section', ['module' => $module->id, 'section' => $section->id]) }}"
           class="truncate font-semibold
                  {{ $isActiveSection ? 'text-bleuone' : 'text-gray-800 hover:text-orangeone' }}">
        ...
        <li>
          <a href="{{ route('stagiaire.module.lecture', ['module' => $module->id, 'section' => $section->id, 'lesson' => $lec->id]) }}"
             class="block px-3 py-2 rounded-lg text-sm font-varela transition
                    {{ $isActiveLesson
                        ? 'bg-orangeone/10 border border-orangeone text-bleuone'
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              @if($stat === 'acquired' || $stat === 'completed')
                <svg class="w-4 h-4 text-vertone" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M20 6L9 17l-5-5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              @elseif($stat === 'incomplete')
                <svg class="w-4 h-4 text-orangeone" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
