...
<head>
  ...
  <title>Oneduc - Buzzer Quiz</title>
  @vite(['resources/css/app.css'])
</head>
<body class="min-h-screen bg-slate-100 flex items-center justify-center p-4">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-6">
    <h1 class="text-xl font-semibold text-bleuone">Rejoindre un Buzzer Quiz</h1>
    <p class="text-sm text-gray-600 mt-1">Entrez le code donné par votre formateur.</p>

    <form method="POST" action="{{ route('buzzer.resolve') }}" class="mt-6 space-y-4">
      @csrf
      <input type="text" name="code" value="{{ old('code') }}" maxlength="12" required autofocus
